...
describe('AuthController (e2e)', () => {
  ...
  describe('register a new user', () => {
    ...
    it('register fails without Input DTO', () => {
      return request(app.getHttpServer()).post('/auth/register').expect(HttpStatus.BAD_REQUEST);
    });
  ...
  describe('refreshing jwt token', () => {
    ...
    it('should successfully get new auth token using refresh token', async () => {
      const loginResponse = await request(app.getHttpServer()).post('/auth/login').send(loginInput);
